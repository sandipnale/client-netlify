  ...
  const onChangeHandler=(event)=>
    ...
    const response=  await registerUser(data);
       if(response.status==201){
        toast.success("Registration complited Please login");
        navigate("/login");

       }else{
      toast.error("Unable to register. plase try again");
       }
          ...
          <div className="card-body p-4 p-sm-5">
            ...
            <form onSubmit={onSubmitHandler}>
               <div className="form-floating mb-3">
                <input type="text" className="form-control" id="floatingName" placeholder="tukaram shinde" name='name' onChange={onChangeHandler} value={data.name} required/>
