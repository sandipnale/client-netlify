...
const Login = () => {
  const { setToken , loadCarData} = useContext(StoreContext);
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      if (response.status === 200) {
       ...
       await loadCarData(response.data.token);
        navigate('/')
      ...
      toast.error("Uable to login please try again");
          ...
          <div className="card border-0 shadow rounded-3 my-5">
            <div className="card-body p-4 p-sm-5">
              ...
              <form onSubmit={onSubmitHandler}>
                ...
                <div className="form-floating mb-3">
                  <input type="password" className="form-control" id="floatingPassword" placeholder="Password" name='password' onChange={onChangeHandler} value={data.value} />
